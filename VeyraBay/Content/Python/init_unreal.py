...
ENTRIES = [
    ...
    ("VB_BuildCity", "4. Stadt bauen (L_VB_City, 12 Bezirke)",
     "Die ganze Stadt als World-Partition-Karte bauen (dauert einige Minuten).",
     "import importlib, vb_common, vb_cityplan, vb_vehicles, vb_setup, vb_city; [importlib.reload(m) for m in (vb_common, vb_cityplan, vb_vehicles, vb_city)]; vb_city.run()"),
    ("VB_OpenCity", "Stadt oeffnen",
     "L_VB_City laden.",
     "import unreal; unreal.get_editor_subsystem(unreal.LevelEditorSubsystem).load_level('/Game/VeyraBay/World/Maps/L_VB_City')"),
